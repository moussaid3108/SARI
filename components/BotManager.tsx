    ...
    <div className="p-4 space-y-4">
      {!showForm ? (
        <button
          ...
          className="w-full py-3 rounded-full bg-violet-600 hover:bg-violet-700 active:bg-violet-800 transition-colors text-white text-[15px] font-bold"
        >
          Create a new bot
        </button>
      ) : (
        <form onSubmit={handleCreate} className="border border-violet-200 bg-violet-50 rounded-2xl p-4 space-y-4">
          ...
          <div className="space-y-1">
            <label className="text-[#536471] text-xs font-medium">
              Username <span className="text-[#8b98a5]">(letters, numbers, _)</span>
            </label>
          ...
          <div className="flex gap-2 pt-1">
            <button
              ...
              className="flex-1 py-2.5 rounded-full border border-[#eff3f4] text-[#536471] hover:border-gray-300 hover:text-[#0f1419] text-sm font-semibold transition-colors"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="flex-1 py-2.5 rounded-full bg-violet-600 hover:bg-violet-700 text-white text-sm font-bold transition-colors"
            >
              Create
            </button>
          </div>
      ...
      <div className="space-y-3">
        {bots.map((bot) => (
          <div key={bot.id} className="border border-[#eff3f4] rounded-2xl p-4 space-y-3 hover:bg-[#f7f9f9] transition-colors bg-white">
            <div className="flex items-center gap-3">
              ...
              <div className="text-right">
                <p className="text-[#0f1419] font-bold text-sm">{bot.posts_count}</p>
                <p className="text-[#8b98a5] text-xs">posts</p>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-2 bg-[#f7f9f9] border border-[#eff3f4] rounded-xl px-3 py-2">
                ...
                <button
                  ...
                  className={`text-xs flex-shrink-0 px-2 py-0.5 rounded-md transition-colors font-medium ${
                    copied === bot.id
